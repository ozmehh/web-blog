...
<?php
	...
	$sql_baglanti= "SELECT id,tarih,kullanici_adi,baslik FROM yazilar ORDER BY tarih DESC";

	$sorgu=mysqli_query($baglanti, $sql_baglanti);
	echo "<table border='1'>";
	echo "<tr><th>ID</th><th>Tarih</th><th> Kullanıcı adı</th><th>Başlık</th><th>SİL</th> <th> DEGİŞTİR</th></tr>";
	while ($satir =mysqli_fetch_array($sorgu))
    {
		
	echo "<tr><td>".$satir['id']."</td><td>".$satir['tarih']."</td><td>".$satir['kullanici_adi']."</td><td>".$satir['baslik']."</td><td>"."<a href='yazisil.php?yaz_id=".$satir['id']."'>"."SİL</a>"."</td><td> "."<a href='yazidegistir.php?yaz_id=".$satir['id']."'>"."DEĞİŞTİR</a></td></tr>";
